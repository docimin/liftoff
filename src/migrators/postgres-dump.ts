...
export const postgresDumpMigrator: Migrator = {
  type: "postgres_dump",
  ...
  async validate(step: Step, context: MigrationContext): Promise<ValidationResult> {
    ...
    if (!service) {
      errors.push("No service specified for postgres_dump step");
  ...
  async execute(step: Step, context: MigrationContext): Promise<StepResult> {
    ...
    // Uses the container's POSTGRES_USER env var
    const dumpResult = await context.source.exec(
      `docker compose -f ${composePath} exec -T ${service} pg_dumpall -U postgres > ${DUMP_PATH}`,
    );

    if (dumpResult.code !== 0) {
      // Fallback: try with POSTGRES_USER from env
      const envResult = await context.source.exec(
        `docker compose -f ${composePath} exec -T ${service} sh -c 'pg_dumpall -U $POSTGRES_USER' > ${DUMP_PATH}`,
      );
      if (envResult.code !== 0) {
        return {
          success: false,
          error: `pg_dumpall failed: ${envResult.stderr || dumpResult.stderr}`,
